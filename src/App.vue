...
const themeOptions: ThemeOption[] = [
  ...
  { id: "celadon", name: "青瓷", group: "温和纸感", dots: ["#6faa95", "#7ba6b8", "#c2a878"] },
  { id: "lakeside", name: "湖畔", group: "温和纸感", dots: ["#5da3a8", "#64a9c4", "#c6ad80"] },
  { id: "pine", name: "松涛夜", group: "温和纸感", dots: ["#7fb59a", "#6fa8a8", "#c4b083"] },
...
const themeGroupOrder = ["材质质感", "温和纸感", "苹果质感", "高饱和霓虹"];
...
// Header avatar: first character of the cloud name (when connected) or the
// local profile name — Chinese characters pass through unchanged.
const avatarChar = computed(() => (accountCloud.value?.name ?? activeProfile.value?.name ?? "KP").trim().charAt(0).toUpperCase());
...
const keyboardRows: KeyItem[][] = [
  ...
  [
    ["Ctrl", 812, 1.5], ["Win", 96], ["Alt", 234, 1.5], ["Space", 8420, 6.5], ["Alt", 144, 1.5], ["Fn", 52], ["Menu", 18], ["Ctrl", 314, 1.5],
...
function backendKeyId(key: KeyItem) {
  ...
  if (key.id === "Ctrl-7") return "ctrl-right";
  if (key.id === "Win-1") return "win-left";
  if (key.id === "Alt-2") return "alt-left";
  if (key.id === "Alt-4") return "alt-right";
...
<p class="ks-note">⌨ 显示的是按下动作，不记录文本 · 托盘菜单或顶栏 ⌨ 按钮可随时开/关 · 拖动后选择任意预设位置可恢复对齐</p>
        ...
        <div class="account-control">
          <button class="avatar-button" aria-label="账号与连接" :aria-expanded="showAccountPanel" :style="{ background: activeProfile?.color }" @click="toggleAccountPanel">{{ avatarChar }}</button>
          <div v-if="showAccountPanel" class="account-popover">
            <div class="account-head">
              <span class="account-avatar" :style="{ background: activeProfile?.color }">{{ avatarChar }}</span>
            ...
            <div class="account-profiles">
              ...
              <div v-if="activeProfile" class="profile-current">
                ...
                <button class="profile-action" style="min-width: 52px; text-align: center;" @click="promptRename(activeProfile)">改名</button>
              </div>
              ...
              <div class="profile-create">
                ...
                <button class="profile-action add" @click="createProfile">＋ 新建</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-show="!showLoginPortal" class="dash-view">
    ...
    <div v-if="showPrivacyPanel" class="privacy-backdrop" role="presentation" @click.self="showPrivacyPanel = false">
      <section class="privacy-dialog" role="dialog" aria-modal="true" aria-labelledby="privacy-title" tabindex="-1" ref="privacyDialog" @keydown.esc="showPrivacyPanel = false">
        ...
        <div class="privacy-storage"><span>本地数据库</span><code>默认在程序所在目录的 keypulse-data 文件夹内 · 可在“设置 → 系统与数据”中更改位置</code></div>
      </section>
    </div>
...
.account-popover { position: absolute; z-index: 12; top: calc(100% + 10px); right: 0; width: 280px; max-height: min(600px, calc(100vh - 90px)); overflow: auto; padding: 14px; border: 1px solid rgba(var(--cyan-rgb), .25); border-radius: 16px; background: rgba(var(--pop-rgb), .98); box-shadow: 0 18px 42px rgba(0, 0, 0, .35); }
.account-profiles { margin-top: 6px; padding-top: 11px; border-top: 1px dashed rgba(var(--line-rgb), .16); }
.account-head { display: flex; align-items: center; gap: 10px; margin-bottom: 9px; }
.account-avatar { display: grid; place-items: center; width: 34px; height: 34px; border-radius: 50%; color: #fff; font-size: 14px; font-weight: 900; background: linear-gradient(135deg, var(--acc-cyan), var(--acc-violet)); }
...
.theme-groups { display: grid; gap: 10px; max-height: 380px; overflow: auto; padding-right: 4px; }
...
.privacy-backdrop { position: fixed; z-index: 20; inset: 0; display: grid; place-items: start center; padding: max(24px, 9vh) 22px 22px; background: rgba(var(--veil-rgb),.72); backdrop-filter: blur(8px); }.privacy-dialog { width: min(100%, 520px); max-height: calc(100vh - max(28px, 9vh) - 46px); overflow: auto; padding: 26px; border: 1px solid rgba(var(--cyan-rgb),.25); border-radius: 20px; background: linear-gradient(145deg, rgba(var(--panel-rgb),.99), rgba(var(--pop-rgb),.99)); box-shadow: 0 28px 80px rgba(0,0,0,.45); }.privacy-dialog-heading { display: flex; align-items: start; justify-content: space-between; gap: 18px; }.privacy-dialog h3 { margin: 5px 0 0; font-size: 22px; letter-spacing: -.05em; }.privacy-close { width: 30px; height: 30px; border: 1px solid rgba(var(--line-rgb),.2); border-radius: 50%; color: var(--tx-soft); background: rgba(var(--line-rgb),.1); cursor: pointer; font-size: 20px; line-height: 1; }.privacy-close:hover { color: #fff; border-color: rgba(var(--cyan-rgb),.65); }.privacy-intro { margin: 22px 0 18px; color: var(--tx-strong); font-size: 13px; line-height: 1.65; }.privacy-points { display: grid; gap: 10px; }.privacy-points article { display: grid; grid-template-columns: 29px 1fr; gap: 11px; padding: 13px; border: 1px solid rgba(var(--line-rgb),.13); border-radius: 12px; background: rgba(var(--ink-rgb),.28); }.privacy-points article > span { color: var(--acc-cyan); font-size: 10px; font-weight: 900; }.privacy-points b, .privacy-points small { display: block; }.privacy-points b { margin-bottom: 4px; color: var(--text-main); font-size: 11px; }.privacy-points small { color: var(--tx-soft); font-size: 10px; line-height: 1.55; }.privacy-notice { display: grid; gap: 5px; margin-top: 14px; padding: 13px; border-left: 3px solid var(--acc-amber); color: var(--tx-soft); background: rgba(var(--amber-rgb),.07); font-size: 10px; line-height: 1.55; }.privacy-notice b { color: var(--acc-amber); }.privacy-storage { display: grid; gap: 6px; margin-top: 17px; color: var(--tx-soft); font-size: 9px; }.privacy-storage code { overflow-wrap: anywhere; color: var(--tx-soft); font-family: ui-monospace, SFMono-Regular, Consolas, monospace; font-size: 9px; }
...
.keyboard-wrap { display: flex; gap: 1.2cqw; padding: 1.6cqw 1.4cqw; border-radius: 1.4cqw; background: rgba(var(--ink-rgb),.42); }.kb-main { display: flex; flex-direction: column; gap: .9cqw; flex: 1 1 auto; min-width: 0; }.keyboard-row { display: flex; gap: .7cqw; min-height: 3.4cqw; }.keycap { display: flex; min-width: 0; flex-direction: column; justify-content: space-between; padding: .38cqw .8cqw .3cqw; border: 1px solid color-mix(in srgb,var(--key-color),white 18%); border-radius: .32cqw; color: #fff; background: linear-gradient(145deg,color-mix(in srgb,var(--key-color),white 9%),var(--key-color)); box-shadow: inset 0 1px rgba(255,255,255,.15),0 5px 10px rgba(0,0,0,.17); transition: transform .18s ease,filter .18s ease; }.keycap:hover { z-index: 2; filter: brightness(1.16); transform: translateY(-4px) scale(1.04); }.keycap span { overflow: hidden; white-space: nowrap; color: rgba(255,255,255,.78); font-size: clamp(5px,.92cqw,9.5px); font-weight: 700; text-overflow: clip; letter-spacing: -.01em; }.keycap b { overflow: hidden; white-space: nowrap; font-size: clamp(5px,.92cqw,9.5px); font-weight: 700; text-overflow: clip; font-variant-numeric: tabular-nums; }.keycap.muted { opacity: .7; }.keycap.blank { opacity: 0; border-color: transparent; box-shadow: none; }
/* function row keys carry 3-char labels (F10-F12): keep them fully readable */
.keyboard-row:first-child .keycap span { font-size: clamp(4.5px,.78cqw,8.5px); letter-spacing: -.03em; }.keycap.side { padding: .25cqw .4cqw; border-radius: .28cqw; min-width: 0; }.keycap.side span { font-size: clamp(5.5px,.95cqw,9px); }.keycap.side b { font-size: clamp(5px,.9cqw,8.5px); }.kb-side { display: flex; gap: 1cqw; flex: 0 0 auto; align-items: start; }
.kb-right-block { display: grid; grid-template-rows: repeat(6, 3.4cqw); grid-template-columns: repeat(3, minmax(3.2cqw, auto)); gap: .9cqw; }
.kb-right-block.num { grid-template-columns: repeat(4, minmax(3.4cqw, auto)); }.keycap.side { border-radius: .28cqw; }
