...
const keyboardRows: KeyItem[][] = [
  [
    ["Esc", 122], ["F1", 84], ["F2", 96], ["F3", 44], ["F4", 58], ["F5", 278], ["F6", 65], ["F7", 72], ["F8", 51], ["F9", 40], ["F10", 32], ["F11", 29], ["F12", 20], ["PrtSc", 12], ["ScrLk", 4], ["Pause", 2],
  ].map(([label, count]) => ({ id: String(label), label: String(label), count: Number(count), muted: label === "PrtSc" || label === "ScrLk" || label === "Pause" })),
  [
...
// Full-104 side zones aligned row-by-row with the main keyboard (like a real
// 104 layout): edit keys beside rows 1-2, direction pad beside rows 3-4,
// numeric keypad from the number row downward.
type SideZoneKey = { id: string; label: string; count: number; w?: number; fn?: boolean };
const S = (id: string, label: string, count: number, w?: number, fn?: boolean): SideZoneKey => ({ id, label, count, w, fn });
type SideRow = { zone: "edit" | "dir" | "num"; row: number; keys: SideZoneKey[] };
const mainSideRows: SideRow[] = [
  { zone: "edit", row: 1, keys: [S("insert", "Ins", 20), S("home", "Home", 34), S("page-up", "PgUp", 26)] },
  { zone: "edit", row: 2, keys: [S("delete", "Del", 96), S("end", "End", 41), S("page-down", "PgDn", 33)] },
  { zone: "dir", row: 3, keys: [S("arrow-up", "↑", 62)] },
  { zone: "dir", row: 4, keys: [S("arrow-left", "←", 88), S("arrow-down", "↓", 120), S("arrow-right", "→", 97)] },
  { zone: "num", row: 1, keys: [S("num-lock", "Num", 8, 1, true), S("numpad-divide", "/", 5), S("numpad-multiply", "*", 6), S("numpad-subtract", "-", 14)] },
  { zone: "num", row: 2, keys: [S("numpad-7", "7", 42), S("numpad-8", "8", 38), S("numpad-9", "9", 45), S("numpad-add", "+", 18, 1, true)] },
  { zone: "num", row: 3, keys: [S("numpad-4", "4", 31), S("numpad-5", "5", 33), S("numpad-6", "6", 29)] },
  { zone: "num", row: 4, keys: [S("numpad-1", "1", 26), S("numpad-2", "2", 28), S("numpad-3", "3", 24), S("numpad-enter", "↵", 22, 1, true)] },
  { zone: "num", row: 5, keys: [S("numpad-0", "0", 44, 2), S("numpad-decimal", ".", 16)] },
];
const sideColumns = [
  { zone: "edit", cells: mainSideRows.filter((r) => r.zone === "edit").flatMap((row) => row.keys.map((key) => ({ ...key, row: row.row, tall: false }))) },
  { zone: "dir", cells: mainSideRows.filter((r) => r.zone === "dir").flatMap((row) => row.keys.map((key) => ({ ...key, row: row.row, tall: false }))) },
  { zone: "num", cells: mainSideRows.filter((r) => r.zone === "num").flatMap((row) => row.keys.map((key, index) => ({ ...key, row: row.row, tall: (row.row === 2 || row.row === 4) && index === row.keys.length - 1 }))) },
];
...
function sideKeyCount(key: KeyItem) {
  if (demoMode.value) return sideKeyColumnCounts[key.id] ?? 0;
    ...
    <section class="content-grid">
      <article class="panel keyboard-panel">
        ...
        <div class="keyboard-wrap">
          ...
          <div class="kb-side">
            <div v-for="col in sideColumns" :key="col.zone" class="kb-right-col" :class="col.zone">
              <div v-for="key in col.cells" :key="key.id" class="kb-cell" :class="{ tall: key.tall }" :style="{ gridRow: sideGridRow(key) }">
                <div class="keycap side" :class="[heatLevel(sideKeyCount(key))]" :style="{ '--key-color': heatColor(sideKeyCount(key)) }" :title="`${key.label}：${formatNumber(sideKeyCount(key))} 次`"><span>{{ key.label }}</span><b>{{ formatNumber(sideKeyCount(key)) }}</b></div>
              </div>
            </div>
          </div>
        </div>
