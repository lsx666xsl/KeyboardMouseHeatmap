...
<table>
  ...
  <tbody>
    ...
    <tr><td>按键可视化</td><td>✅ 已实现</td><td>三风格 + 位置/大小/透明度/拖动自由定制</td></tr>
    <tr><td>主题配色</td><td>✅ 6 套</td><td>新增苹果石墨与星光浅色，浅色主题全面适配</td></tr>
    <tr><td>迷你统计窗与数据迁移</td><td>✅ 已实现</td><td>右上角实时小卡片；数据库位置可切到程序目录</td></tr>
    <tr><td>趣味功能</td><td>✅ 已实现</td><td>打字音效 / 成就提示 / 每日足迹卡 / 局域网 PK（云端中继参考见 server/）</td></tr>
    <tr><td>自动更新</td><td>🔄 已实现待发版验证</td><td>updater 插件 + GitHub Releases，tag 推送自动构建发布；流程见 docs/自动更新与发布.md</td></tr>
...
- 总量、活跃时段、高频按键和趋势图
- 活泼、丰富、年轻化的可视化主题
- 12 套可切换配色（含 6 套设计师高饱和主题），选择会记住
- 实时按键可视化：按下按键时屏幕显示按键（录屏/演示/教学友好），三风格、六位置、三档大小可调
- 系统托盘后台运行、暂停/继续记录
- 本地离线运行，不保存完整输入文本
...
- [自动更新与发布](docs/自动更新与发布.md)：自动更新机制、签名密钥与 Release 发布流程
